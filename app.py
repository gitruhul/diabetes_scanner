...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://retinopathy:password@localhost:3306/retinopathy2'
...
def preprocess_images(images):
    ...
    for image in images:
        ...
        if len(resized_image.shape) == 2:  # Grayscale image
            ...
            if resized_image.shape[2] == 1:  # Single-channel image (unlikely, but just in case)
                resized_image = cv2.cvtColor(resized_image, cv2.COLOR_GRAY2RGB)
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        image_path = os.path.join('uploads', filename)
        image = cv2.imread(image_path)
        result, class_label = analyze_image(image)  # Modify analyze_image to also return the class label
        flash(result, 'success')
        classes = ['mild','Moderate','No_DR','proliferate_DR','Severe']
        return render_template('output.html', result=result, class_label=classes[class_label])  # Pass the class_label to the template
    return render_template('upload.html', form=form)
